...
/*
 * Create dailytransactions table id transactionID Items totalValue Date Between given dates, compute the total value for each day For each day, - Get all transactions for a day - Add the totalValue column - put the totalValue computed for a day in transantionsTable Now, you will be ready with Totalvalue for each day in transactionsTable
 */
function computeTotalValueForEachDate($dateFrom, $dateTo) {
	...
	// Step #1
	$allDaysInGivenDates = PDO_FetchAll ( "SELECT Date FROM  dailytransactions WHERE Date BETWEEN :Date AND :Date2 ", array (
			"Date" => $dateFrom,
			"Date2" => $dateTo 
	) );
	...
	foreach ( $listOfUniqueDates as $eachDay ) {
		...
		// Step #3
		$allTransactionsForDay = PDO_FetchAll ( "SELECT  * FROM  dailytransactions WHERE Date =:Date", array (
				"Date" => $eachDay 
		) );
		$sum = 0;
...
function updateTransactionsTable($dateFrom, $sum) {
	...
	$dateInTable = PDO_FetchAll ( "SELECT * FROM transactions WHERE OrderDate =:Date", array (
			"Date" => $dateFrom 
	) );
	if (count ( $dateInTable ) > 0) {
		// Do an update
		PDO_Execute ( "UPDATE transactions SET OrderValue =:OrderValue WHERE OrderDate =:OrderDate", array (
				"OrderDate" => $dateFrom,
				"OrderValue" => $sum 
		) );
	} else {
		// Do insert
		PDO_Execute ( "INSERT INTO transactions (OrderDate,OrderValue) VALUES (:OrderDate, :OrderValue)", array (
				"OrderDate" => $dateFrom,
				"OrderValue" => $sum 
		) );
	}
...
function computeAndDisplayEachDayTotalSalesAsGraph($dateFrom, $dateTo) {
	$All = PDO_FetchAll ( "SELECT  * FROM  transactions WHERE OrderDate BETWEEN :Date AND :Date2 ", array (
			"Date" => $dateFrom,
			"Date2" => $dateTo 
	) );
	// print_r($All);
