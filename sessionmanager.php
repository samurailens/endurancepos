...
if ($_SERVER ['REQUEST_METHOD'] == 'POST') {
	...
	if (isset ( $_POST ['signupsubmitbutton'] )) {
		/*
		 * this is a signup request, because user clicked the signup submit button echo "singup";
		 */
		handleSignup ();
...
function handleSignup() {
	...
	PDO_Execute ( "INSERT INTO user(email,fname,password,phone) VALUES (:email,:fname,:password,:phone )", array (
			"email" => $email,
			"fname" => $fname,
			"password" => $password,
			"phone" => $phone 
...
function SignIn() {
	/*
	 * $val = $_POST['passwordsignin']; //"rasmuslerdorf"; $hash=password_hash($val, PASSWORD_DEFAULT); echo "HASH " . $hash; if (password_verify($val, $hash)) { echo 'Password is valid!'; } else { echo 'Invalid password.'; }
	 */
	$enteredpassword = $_POST ['passwordsignin'];
	$enteremail = $_POST ['emailsignin'];
	
	$result = PDO_FetchAll ( "SELECT password FROM user WHERE email = :email", array (
			"email" => $enteremail 
	) );
	$hash = '';
